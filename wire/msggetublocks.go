...
func (msg *MsgGetUBlocks) AddBlockLocatorHash(hash *chainhash.Hash) error {
	if len(msg.BlockLocatorHashes)+1 > MaxBlockLocatorsPerMsg {
		str := fmt.Sprintf("too many block locator hashes for message [max %v]",
			MaxBlockLocatorsPerMsg)
		return messageError("MsgGetUBlocks.AddBlockLocatorHash", str)
	}
...
func (msg *MsgGetUBlocks) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockLocatorsPerMsg {
		...
		return messageError("MsgGetUBlocks.BtcDecode", str)
	}
...
// This is part of the Message interface implementation.
func (msg *MsgGetUBlocks) BtcEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	count := len(msg.BlockLocatorHashes)
	if count > MaxBlockLocatorsPerMsg {
		str := fmt.Sprintf("too many block locator hashes for message "+
			"[count %v, max %v]", count, MaxBlockLocatorsPerMsg)
		return messageError("MsgGetUBlocks.BtcEncode", str)
	}
...
func (msg *MsgGetUBlocks) MaxPayloadLength(pver uint32) uint32 {
